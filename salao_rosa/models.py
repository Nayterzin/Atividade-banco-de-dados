...
class Funcionario(models.Model):
    ...
    tipo = models.CharField(max_length=20, choices=TipoFuncionario.choices)
    cpf = models.CharField(max_length=14, unique=True, validators=[RegexValidator(r'^\d{3}\.\d{3}\.\d{3}-\d{2}$')])
...
class EspecializacaoCabeleireira(models.Model):
    funcionario = models.OneToOneField(Funcionario, on_delete=models.CASCADE, primary_key=True, db_column='idfun')
...
class Agenda(models.Model):
    ...
    funcionario = models.ForeignKey(Funcionario, on_delete=models.RESTRICT, db_column='idfun')
    cliente = models.ForeignKey(Cliente, on_delete=models.RESTRICT, db_column='idcliente')
    procedimento = models.ForeignKey(Procedimento, on_delete=models.RESTRICT, db_column='idprocedimento')
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['funcionario', 'data_hora'], condition=~models.Q(status='cancelado'), name='agenda_funcionario_horario')
        ]
